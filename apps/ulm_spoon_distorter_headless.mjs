// Headless ULM SPOON DISTORTER driver: boots the sealed machine + the cart the way
// sealed-loader.js does, runs N frames and writes the whole 400x280 physical
// screen (the scene is a fullscreen overscan plane, so the borders ARE content).
//
...
//
//   node apps/ulm_spoon_distorter_headless.mjs [outdir]
...
const { memory, machine, demo } = await boot("docs/demo-ulm_spoon_distorter.wasm");
...
async function shot(path) {
    ...
    for (let y = 0; y < VH; y++)
        for (let x = 0; x < VW; x++)
            for (let c = 0; c < 3; c++)
                buf[hdr.length + (y * VW + x) * 3 + c] = img[(y * W + x * XS) * 3 + c];
...
const out = process.argv[2] || "/tmp/ulm_spoon_distorter";
await mkdir(out, { recursive: true });
if (W !== 800 && W !== 400) console.log(`  note: physical width ${W}`);
// the intro wave is 7282 steps = ~1456 iterations = ~35 s at 60fps; sample
// both the intro and the main sequence
for (const [n, name] of [[1, "0001"], [300, "0301"], [900, "1201"], [1500, "2701"], [1800, "4501"]]) {
    run(n);
    await shot(`${out}/${name}.ppm`);
}

// Frame cost: the cart's frame() plus the machine composite of its plane
