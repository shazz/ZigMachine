// Headless CUDDLY STARWARS driver: boots the sealed machine + the cart the way
// sealed-loader.js does, runs N frames and writes the visible 320x200 screen.
//
...
//
// Frame N here is go() call N of the original (init runs at boot, and each
// demo.frame is one update + render), so shots line up with a reference render.
//
//   node apps/cuddly_starwars_headless.mjs [outdir]
...
const { memory, machine, demo } = await boot("docs/demo-cuddly_starwars.wasm");
...
const out = process.argv[2] || "/tmp/cuddly_starwars";
await mkdir(out, { recursive: true });
if (W !== 800 && W !== 400) console.log(`  note: physical width ${W}`);
// 290: mid logo flash (fade starts at 280); 400: a later starwars page
for (const f of [1, 60, 290, 400]) {
    run(f - frames);
    await shot(`${out}/${String(f).padStart(3, "0")}.ppm`);
}

// Frame cost: the cart's frame() plus the machine composite of its plane (HBLs
// included), which is the CPU half of a real frame.
const N = 600;
