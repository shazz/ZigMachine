// ---------------------------------------------------------------------------
// tutorial_steps.rs: the tutorial screen, stopped at any step of docs/TUTORIAL.md.
//
// The Rust twin of apps/zig/scenes/tutorial_steps.zig — see that file for WHY
// this is a copy of tutorial.rs rather than a flag inside it (short version:
// every snippet in docs/TUTORIAL.md is quoted verbatim from tutorial.rs, and
// gates in there would make the tutorial's own code untrue). The copy is held to
// the original by apps/tutorial_steps_check.mjs, which asserts this cart at
// step 7 renders frames byte-identical to demo-rust-tutorial.wasm.
//
...
//
// Build: bash apps/rust/build.sh tutorial_steps
// Run:   docs/index.html?demo=demo-rust-tutorial_steps.wasm&step=3
// ---------------------------------------------------------------------------
#![no_std]
#![no_main]
...
static mut STEP: u32 = LAST_STEP;
static mut RESTAGE: bool = true;

// SAFETY for STEP/RESTAGE: the machine is single-threaded and calls the cart's
// exports one at a time, exactly as SCREEN above relies on.
fn step() -> u32 {
    unsafe { STEP }
}

// Everything tutorial.rs does in boot(), gated by how far the reader has got.
fn stage() {
    if step() >= 7 {
        zigmachine_music::request_song("sos.sndh"); // step 7: music
    }
    let s = screen();
    // SAFETY: REG_BACKGROUND is a u32 register inside the video region.
    unsafe { ((s.base + REG_BACKGROUND) as *mut u32).write(rgba(0, 0, 0)) };
    if step() < 2 {
        return; // step 1: an empty cart that boots, and nothing else
    }

    let pal = s.palette(); // step 2: a plane and a palette
    ...
    let sky = pal[SKY as usize];

    s.fill_rect(0, 0, W, H, SKY);
    if step() >= 3 {
        s.draw_floor(); // step 3: the checkerboard floor
    }

    // step 5: arm plane 0's HBL for the copper bar; 0 = no HBL before that.
    let armed: u16 = if step() >= 5 { 1 } else { 0 };
    if armed == 1 {
        s.copper = [sky; H];
    }
    // SAFETY: plane 0's u16 HBL registers, inside the video region.
    unsafe {
        ((s.base + REG_FB_HBL_ID) as *mut u16).write(armed);
...
pub extern "C" fn boot() {
    ...
    unsafe { RESTAGE = true }; // stage() runs on the first frame, once the step is known
}
...
pub extern "C" fn frame(_elapsed_ms: f32) {
    if unsafe { RESTAGE } {
        stage();
        unsafe { RESTAGE = false };
    }
    if step() < 4 {
        return; // steps 1-3 are static: stage() drew them already
    }
    let s = screen();
    s.fill_rect(0, 0, W, FLOOR_Y, SKY); // wipe last frame's sky
    s.move_block(); // step 4: the block moves
...
#[no_mangle]
pub extern "C" fn setShadeMode(m: u32) -> i32 {
    if m < 1 || m > LAST_STEP {
        return 0;
    }
    unsafe {
