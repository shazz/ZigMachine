...
const MAP_W: f32 = 175 * 32; // TMXLayer.width: the x limit of checkCollision
...
pub const Charly = struct {
    ...
    pub fn update(self: *Charly, in: Input, map: anytype) void {
        if (in.left) {
    ...
    // `falling` only ever comes from bumping a ceiling).
    fn collide(self: *Charly, map: anytype) void {
        const b = self.box();
        const qx = if (self.vx < 0) @trunc(b.left + self.vx) else @ceil(b.right - 1 + self.vx);
        ...
        if (qx <= 0 or qx >= MAP_W) {
            ...
            x_kind = firstSolid(map, .{ qx, @ceil(b.bottom - 1) }, .{ qx, @trunc(b.top) });
            x_hit = x_kind != 0;
        }
        const near = if (self.vx < 0) @trunc(b.left) else @ceil(b.right - 1);
        const far = if (self.vx < 0) @ceil(b.right - 1) else @trunc(b.left);
        ...
        if (x_hit and x_kind != PLATFORM) self.vx = 0;
    }
