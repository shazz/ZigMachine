...
pub const Demo = struct {
    ...
    col2: u32,
    grad_row: [SCROLL_H]u8, // scrollbg_canvas: one index per row, 0 = never drawn
    ...
    /// calc_scroll_background: two gradient slices, re-chosen every 12 s.
    fn calcScrollBackground(self: *Demo) void {
        ...
        for (0..GRAD_SLICE) |k| {
            self.grad_row[GRAD_TOP + k] = GRAD_BASE + @as(u8, @intCast(self.col1 + k));
            self.grad_row[GRAD_LOW + k] = GRAD_BASE + @as(u8, @intCast(self.col2 + k));
        }
    }
    ...
    fn drawScroller(self: *Demo, view: blit.Dst) void {
        fillRows(view, SCROLL_Y, SCROLL_H, BAND);
        const box = view.window(SCROLL_X - CROP_X, SCROLL_Y, SCROLL_W, SCROLL_H);
        for (self.grad_row, 0..) |index, row| {
            if (index != 0) @memset(box.buf[row * box.stride ..][0..box.w], index);
        }
        for (self.letters) |l| {
